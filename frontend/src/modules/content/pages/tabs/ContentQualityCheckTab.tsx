...
import {
  VehicleCardType,
  useGetQualityCheckerVehiclesQuery,
} from '../../api/getQualityCheckerVehicles';
...
import { usePermissions, PERMISSIONS } from '@/modules/auth';
...
const ContentQualityCheckTab = () => {
  ...
  const isPhotoBoxReady =
    data?.qualityCheckerVehicles?.photoBoxStatus === PHOTOBOX_STATUS.READY;
  const noPhotosInBox =
    data?.qualityCheckerVehicles?.vehicles?.count === 0 && !filterUsed.current;
          ...
          return (
            <PageMessage messages={PAGE_MESSAGES_PHOTO_BOX_CREATING}>
              <div className="mt-12 flex">
                ...
                <Button variant="secondary">{t('content.contactSales')}</Button>
              </div>
            </PageMessage>
          );
        }
...
export default ContentQualityCheckTab;
